...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <aside 
      className={`bg-white border-r border-sbd-lightgray flex flex-col transition-all duration-300 ${
    ...
    >
      ...
      <div className="h-14 bg-sbd-yellow flex items-center justify-between px-4">
        {!isCollapsed && (
          <div className="text-sbd-charcoal font-montserrat font-bold">
            <span>Tool Doctor</span>
          </div>
        )}
        <button 
          onClick={toggleCollapse}
          className="p-1 rounded-md hover:bg-sbd-yellow/70 transition-colors"
        ...
        {!isCollapsed && (
          <>
            ...
            <input
              ...
              placeholder="e.g., DCD778"
              className="w-full p-2 border border-sbd-midgray/30 rounded-md focus:outline-none focus:ring-2 focus:ring-sbd-yellow/50"
            />
          </>
        )}
      </div>
